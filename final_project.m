function final_project
    ...
    %Global Variables, needed in the UI controls
    kClusts = 4;
    globalNext = 0;

    % Construct the UI components: buttons, fields, slider
    %Stuff along the right edge
    ...
    hremove = uicontrol('Style','pushbutton','String','REMOVE','Position',[400,15,75,30],'Callback',@remove_regions);
    ...
    disp('Keep in mind nothing will work until you supply this directory(put it in quotes)');
    ...
    %%  Get some general info that I will need later about the images
    ...
    clustSlices = [];
    numCents = 0;
    clustered = 0; %variable determining whether or not the images were clustered
    reClust = 0;

    % Edge detecting variables
    edgedSlices = []; % structure for holding the edges for each slice
    edged = 0;  %variable determining whether the variables were clustered
    xverts = [];
    yverts = [];
    zverts = [];
	...
	function subSelect
		...
		%get the new dimensions based on the croping
		newWidth = (sp(3) - sp(1)) + 1;
		newHeight = (sp(4) - sp(2)) + 1;
		%crop the new image
		newIm = im(sp(2):sp(4), sp(1): sp(3));  
		
		%figure; image(MM); axis image
        dispTitle = ['Im: ',names(curSlice,:),',  # ',num2str(curSlice),', Inst # ',num2str(sortInstNums(curSlice))];
        imshow(newIm,[]), title(dispTitle);
	...
	function grayCluster
		...
		disp('This is important, dont screw this up!');
        disp('Use the cluster numbers displayed above each image');
        disp('Please put selections in brackets: [1,2,3]');
    ...
    function crop_Callback(source, eventdata)
		%Crop a single image
		disp('Use middle number: #');
        curSlice = input('Please choose initial image to crop:  ');
		
        if subSelected == 1
            disp('You have already Cropped one image, this will redo that, just letting you know');
        elseif subSelected == 2
            disp('You have already cropped everything...')
            disp('If you do this and do not crop everything right after this will produce results later');
        end
	...
	function cropAll_Callback(source, eventdata)
		...
		if subSelected == 2
            disp('You have already cropped everything, just letting you know');
        end
   ...
   function kslider_Callback(source,eventdata) 
      ...
      %warning message
      if clustered > 0
        disp('Note if you change this in between the single cluster and clustering everything');
        disp('It WILL PRODUCE WEIRD RESULTS, you have been warned!');
      end
	...
	function clusterButton_Callback(source,eventdata) 
        ...
        if(subSelected == 2)
            ...
            if clustered == 2
                ...
                if (reClust ~= 0) && (reClust ~= 1)
                    while (reClust ~= 0) && (reClust ~= 1)
                        disp('oops we only accept: reclustering(1) or starting over(0), try again.');
                        reClust = input('Are you reclustering(1) or starting over(0)?  ');
                    end
                end
            end
  	...
  	function clusterAll_Callback(source,eventdata) 
        if clustered > 0
            ...
            if clustered == 2
                disp('Looks like you have already done all the clustering, just letting you know');
            end
            
            clusterAll
        else
            disp('Looks like you have not done the initial clustering, please go back and do that');
        end
	end
	...
	function getEdges_Callback(source,eventdata)
		if clustered == 2
           ...
           disp('Looks like you have not done the clustering, please go back and do that'); 
        end
    end
    ...
    function remove_regions(source,eventdata)
        ...
        if edged == 0
            disp('You did not edged the images yet, go back and do that!');
        else   
            disp('Warning this can only be undone by redecting the edges');
            disp('Use middle number: #');
            curSlice = input('Which image do you want to edit? ');
            ...
            dispTitle = ['Newly Edged: ',names(curSlice,:),', # ',num2str(curSlice),', Inst # ',num2str(sortInstNums(curSlice))];
            imshow(edgedSlices(:,:,curSlice),[]), title(dispTitle);
        end
    end
	...
	function nextbutton_Callback(source,eventdata) 
		if dispSlices == 0
			...
			switch dispSlices
                ...
                case 4
                    dispTitle = ['Edge Slice: ',dispTitle];
                    imshow(edgedSlices(:,:,curSlice),[]), title(dispTitle);
				end
		end
	end 
    ...
    function dispOrig_Callback(source, eventdata)
        dispSlices = 1;
        curSlice = 1;
        dispTitle = ['Originals: ',names(curSlice,:),', # ',num2str(curSlice),', Inst # ',num2str(sortInstNums(curSlice))];
        imshow(slices(:,:,curSlice),[]), title(dispTitle);
    end
	...
	function dispClust_Callback(source, eventdata)
		if clustered == 0
			...
			dispSlices = 3;
            curSlice = 1;
            dispTitle = ['Clustered: ',names(curSlice,:),', # ',num2str(curSlice),', Inst # ',num2str(sortInstNums(curSlice))];
            imshow(clustSlices(:,:,curSlice),[]), title(dispTitle);
		end
    end
